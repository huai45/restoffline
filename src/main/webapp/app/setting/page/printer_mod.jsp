...
String sql = " select printer , ip  from td_printer a  where printer = ?   ";
JdbcTemplate jdbcTemplate = (JdbcTemplate)GetBean.getBean("jdbcTemplate");
List printers = jdbcTemplate.queryForList(sql,new Object[]{ id});
Map printer = new HashMap();
if(printers.size()>0){
	printer = (Map)printers.get(0);
}
%>
...
<div id="center" data-options="region:'center',border:false,style:{borderWidth:0}" style="padding:0px;background:#FFF;">
    ...
    <div id="north" style="height:50px;padding-left:20px;line-height:50px;border-bottom:solid 0px #CCCCCC;font-size:20px;">
        <div id="" style="height:48px;width:100%;line-height:50px;">
            ...
            <span>编辑打印机信息</span><span style="margin-left:150px;font-size:16px;">当前打印机名称 ： <%= printer.get("printer") %></span>
        </div>
    ...
    <div id="center" style="">
        <div style="float:left;width:500px;background:#fff;margin-left:0px;margin-top:0px;">
		    ...
		    <div style="margin-left:100px;margin-bottom:5px;font-size:18px;">IP地址</div>
		    <div style="margin-left:100px;margin-bottom:15px;">
	            <input id="ip" type="text" value="<%= printer.get("ip") %>" style="border:solid 1px #CCCCCC;padding:0px;margin:0px;line-height:32px;width:300px; height:32px;font-size:20px; " />
	        </div>
