...
String sql = " select  oper_staff_name, ROUND(sum(price*(count-back_count)),2) count, ROUND(sum(price*(count-back_count-free_count)*pay_rate/100),2) real_count "+ 
  " from th_bill_item a where oper_time >= ? and oper_time <= ? group by  oper_staff_name order by count DESC ; ";
ut.log(" sql :" +sql);
...
$(document).ready(function(){
     ...
     $("#queryStaffMoney").addClass("left_menu_sel");
	...
	$("#queryBtn").click(function(){
        ...
        document.location.href="/transPage?page=/query/queryStaffMoney&start_date="+start_date+"&end_date="+end_date;
	});
...
<div id="center" data-options="region:'center',border:false,style:{borderWidth:0}" style="padding:0px;background:#FFF;">
    ...
    <div id="center">
	    ...
	    <div style="margin-left:10px;margin-top:20px;margin-bottom:0px;font-size:22px;"><%= "员工点菜统计" %>
        ...
        <div class="audit_list" style="width:930px;">
            <div class="order_head" style="">
	            ...
	            <div style="width:150px;" >员工ID</div>
	            <div style="width:150px;" >员工姓名</div>
	            <div style="width:200px;" >总销售额 (元)</div>
            ...
            <%  double total1 = 0;
                ...
                for(int i=0;i<details.size();i++){
                   ...
                   staffData.put("oper_staff_id","-");
                   total1 = total1 + Double.parseDouble(staffData.get("count").toString());
                   total2 = total2 + Double.parseDouble(staffData.get("real_count").toString());
            %>
            <div class="order_tr" id="<%= staffData.get("bill_id") %>" >
                <div style="float:left;padding-top:1px;width:40px;text-align:center;" >
	                &nbsp;
	            </div>
	            <div class="bluefont1 bluefont" style="width:150px;" ><%= staffData.get("oper_staff_id") %></div>
	            <div class="bluefont1 bluefont" style="width:150px;"><%= staffData.get("oper_staff_name") %></div>
	            <div style="width:200px;" ><%= ut.getDoubleString(Double.parseDouble(staffData.get("count").toString())) %></div>
	            <div style="width:300px;" ><%= ut.getDoubleString(Double.parseDouble(staffData.get("real_count").toString())) %>&nbsp;</div>
	        </div>
	        <% } %>
