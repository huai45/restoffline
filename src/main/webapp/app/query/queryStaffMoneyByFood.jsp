...
String table = "td_android_user";
String sql = "select food_name,price, sum(COUNT)  total , sum(( count-BACK_COUNT-FREE_COUNT)*price) money from th_bill_item "+
    " where oper_time >= ? and oper_time <= ? and OPER_STAFF_NAME = ? "+
    " group by FOOD_NAME having sum(COUNT) > 0 order by sum(count) desc";  
...
List staffList = jdbcTemplate.queryForList(" select user_id,username from "+table+" where 1 = 1 order by user_id ",
    new Object[]{ } );
...
$(document).ready(function(){
     ...
     $("#queryStaffMoneyByFood").addClass("left_menu_sel");
	...
	$("#queryBtn").click(function(){
        ...
        document.location.href="/transPage?page=/query/queryStaffMoneyByFood&start_date="+start_date+"&end_date="+end_date+"&staff_name="+staff_name;
	});
...
<div id="center" data-options="region:'center',border:false,style:{borderWidth:0}" style="padding:0px;background:#FFF;">
    <div id="" style="height:50px;width:95%;padding-left:0px;line-height:50px;border-bottom:solid 0px #CCCCCC;font-size:20px;">
        <div id="" style="height:50px;width:100%;line-height:50px;border-bottom:solid 1px #CCCCCC;font-size:20px;">
	        ...
	        <div style="float:left;height:50px;line-height:30px;margin-right:15px;">
	            <select id="staff_name" style="float:left;margin-top:11px;border:solid 1px #CCCCCC;padding:0px;line-height:30px;width:100px; height:30px;font-size:18px; ">
					<% 
					for(int i=0;i<staffList.size();i++){
						Map staff = (Map)staffList.get(i);
					%>
					<option value="<%= staff.get("username") %>"><%= staff.get("username") %></option>
					<% 
					}
					%>
				</select>
    ...
    <div id="center">
	    ...
	    <div style="margin-left:10px;margin-top:20px;margin-bottom:0px;font-size:22px;"><%= "员工("+staff_name+")点菜统计" %>
        ...
        <div class="audit_list" style="width:930px;">
            <div class="order_head" style="">
	            ...
	            <div style="width:200px;" >菜品名称</div>
	            <div style="width:100px;" >单价 (元)</div>
	            <div style="width:200px;" >实际销售量</div>
	            <div style="width:300px;" >实际销售额 (元) : 去掉赠送和折扣</div>
	        </div>
	        <div id="show_list">
            <% 
                double total1 = 0;
                ...
                for(int i=0;i<details.size();i++){
                   ...
                   total1 = total1 + Double.parseDouble(staffData.get("total").toString());
                   total2 = total2 + Double.parseDouble(staffData.get("money").toString());
            %>
            <div class="order_tr" id="<%= staffData.get("oper_date") %>" >
                <div style="float:left;padding-top:1px;width:40px;text-align:center;" >
	                &nbsp;
	            </div>
	            <div class="bluefont1 bluefont" style="width:200px;" ><%= staffData.get("food_name") %></div>
	            <div style="width:100px;" ><%= staffData.get("price") %></div>
	            <div style="width:200px;" ><%= Double.parseDouble(staffData.get("total").toString()) %></div>
	            <div style="width:300px;" ><%= Double.parseDouble(staffData.get("money").toString()) %>&nbsp;</div>
	        </div>
	        <% } %>
