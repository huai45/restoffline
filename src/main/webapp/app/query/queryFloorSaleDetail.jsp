...
$(document).ready(function(){
     ...
     $("#floorsaildetailpage").addClass("left_menu_sel");
	...
	$("#queryBtn").click(function(){
        ...
        document.location.href="/transPage?page=/query/queryFloorSaleDetail&start_date="+start_date+"&end_date="+end_date;
	});
	...
	$("#exportBtn").click(function(){
		...
		form.attr("action","/file/exportFloorSaleDetailFile.html");

		var input1=$("<input>");
		input1.attr("type","hidden");
		input1.attr("name","start_date");
		input1.attr("value",start_date);
		form.append(input1);
		
		var input2=$("<input>");
		input2.attr("type","hidden");
		input2.attr("name","end_date");
		input2.attr("value",end_date);
		form.append(input2);
...
<div id="center" data-options="region:'center',border:false,style:{borderWidth:0}" style="padding:0px;background:#FFF;">
    ...
    <div id="center">
	    ...
	    <div style="margin-left:10px;margin-top:20px;margin-bottom:0px;font-size:22px;">档口楼层销售销售统计
	        <span style="font-size:16px;margin-left:15px;">(  <%= start_date %>  至  <%= end_date %> )</span>
	        <div id="exportBtn" align="center" class="btn" style="float:right;background:#80397B;color:#FFFFFF;margin-top:8px;margin-right: 18px; " onselectstart='return false'> 导出excel </div>
        ...
        <div class="audit_list" style="width:930px;">
            <div class="order_head" style="">
	            ...
	            <div style="width:150px;" >类别</div>
	            <div style="width:150px;" >楼层</div>
	            <div style="width:200px;" >销售金额 (元)</div>
	        </div>
	        <div id="show_list">
            <%  double total1 = 0;
                double total2 = 0;
                for(int i=0;i<details.size();i++){
                   Map floorSale = (Map)details.get(i);
                   total1 = total1 + Double.parseDouble(floorSale.get("MONEY").toString().replaceAll(",",""));
            %>
            <div class="order_tr" >
                <div style="float:left;padding-top:1px;width:40px;text-align:center;" >
	                &nbsp;
	            </div>
	            <div class="bluefont1 bluefont" style="width:150px;" ><%= floorSale.get("GROUPS") %></div>
	            <div class="bluefont1 bluefont" style="width:150px;"><%= floorSale.get("FLOOR") %></div>
	            <div style="width:200px;" ><%= floorSale.get("MONEY") %></div>
	        </div>
	        <% } %>
